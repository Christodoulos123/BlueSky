import json
import networkx as nx
import matplotlib.pyplot as plt
import matplotlib.patches as mpatches

# === Config ===
input_path = "/home/christodoulos/Documents/GitHub/BlueSky/output/timeStamp_Posts/info/interactions.json"  # Replace with your actual path
output_mapping_path = "/home/christodoulos/Documents/GitHub/BlueSky/output/timeStamp_Posts/info/handle_id_map.json"  # Output mapping file
focus_user = "rchusid.bsky.social"

# === Load interactions ===
with open(input_path, "r") as f:
    interactions = json.load(f)

# === Filter for only interactions involving the focus user ===
filtered = [
    i for i in interactions
    if i["actor_handle"] == focus_user or i["target_handle"] == focus_user
]

print(f"Found {len(filtered)} interactions involving '{focus_user}'")

# === Get all handles involved ===
handles = set(i["actor_handle"] for i in filtered) | set(i["target_handle"] for i in filtered)
handle_to_id = {handle: idx for idx, handle in enumerate(sorted(handles))}
id_to_handle = {v: k for k, v in handle_to_id.items()}

# === Save handle-to-ID mapping ===
with open(output_mapping_path, "w") as f:
    json.dump(handle_to_id, f, indent=2)
print(f"Saved handle-to-ID mapping to '{output_mapping_path}'")

# === Build directed graph ===
G = nx.Graph()
interaction_colors = {
    "like": "blue",
    "repost": "green",
    "reply": "orange",
    "quote": "purple"
}

for i in filtered:
    actor = handle_to_id[i["actor_handle"]]
    target = handle_to_id[i["target_handle"]]
    interaction_type = i["interaction"]
    color = interaction_colors.get(interaction_type, "gray")

    G.add_edge(
        actor,
        target,
        label=interaction_type,
        color=color
    )

# === Draw graph ===
pos = nx.spring_layout(G, k=1.0, seed=42)
edge_colors = [d['color'] for u, v, d in G.edges(data=True)]

plt.figure(figsize=(14, 10))
nx.draw(
    G,
    pos,
    with_labels=True,
    node_color='lightblue',
    node_size=800,
    font_size=9,
    arrows=True,
    edge_color=edge_colors,
    connectionstyle='arc3,rad=0.1'
)

legend_patches = [
    mpatches.Patch(color='blue', label='Like'),
    mpatches.Patch(color='green', label='Repost'),
    mpatches.Patch(color='orange', label='Reply'),
    mpatches.Patch(color='purple', label='Quote')
]
plt.legend(handles=legend_patches, loc='lower left', fontsize=10)

plt.title(f"Interaction Graph for {focus_user}", fontsize=16)
plt.axis('off')
plt.tight_layout()
plt.savefig("/home/christodoulos/Documents/GitHub/BlueSky/output/timeStamp_Posts/info/User_graph.png", dpi=300)
print("Graph saved as 'eladn_interaction_graph.png'")
